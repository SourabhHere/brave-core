...
std::string WalletProviderRegionsToString(
  ...
  std::string string;
  base::JSONWriter::Write(wallet_provider_regions_dict, &string);

  return string;
}

base::flat_map<std::string, ledger::mojom::RegionsPtr>
StringToWalletProviderRegions(const std::string& json) {
  const auto value = base::JSONReader::Read(json);
...
void State::SetRewardsParameters(const mojom::RewardsParameters& parameters) {
  ...
  ledger_->ledger_client()->SetStringState(
      ...
      WalletProviderRegionsToString(parameters.wallet_provider_regions));
}
...
State::GetWalletProviderRegions() {
  return StringToWalletProviderRegions(ledger_->ledger_client()->GetStringState(
      kParametersWalletProviderRegions));
}
