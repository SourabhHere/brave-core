...
struct ParamInfo final {
  std::string locale;
  bool should_sanction_un_m49_area_codes;
  std::string expected_language_code;
  std::string expected_country_code;
  bool expected_is_ofac_sanctioned;
} kTests[] = {
    {{}, false, kDefaultLanguageCode, kDefaultCountryCode, false},
    {{}, true, kDefaultLanguageCode, kDefaultCountryCode, false},

    // ISO 639-1 language codes, see
    // https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes.

    {"en", false, "en", kDefaultCountryCode, false},
    {"en.UTF8", false, "en", kDefaultCountryCode, false},
    {"en", true, "en", kDefaultCountryCode, false},
    {"en.UTF8", true, "en", kDefaultCountryCode, false},
    ...
    {"en_US.UTF8", false, "en", "US", false},
    {"en_US", true, "en", "US", false},
    {"en_US.UTF8", true, "en", "US", false},

    {"en_840", false, "en", "840", false},
    {"en_840.UTF8", false, "en", "840", false},
    {"en_840", true, "en", "840", false},
    {"en_840.UTF8", true, "en", "840", false},

    {"az_Latn_AZ", false, "az", "AZ", false},
    {"az_Latn_AZ.UTF8", false, "az", "AZ", false},
    {"az_Latn_AZ", true, "az", "AZ", false},
    {"az_Latn_AZ.UTF8", true, "az", "AZ", false},
    ...
    // ISO 639-1 language codes and UN M49 area codes, see
    // https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes and
...
TEST_P(OFACSanctionLocaleUtilTest, Locale) {
  ...
  // Act
  const std::string language_code = icu::GetLanguageCode(locale);

  const std::string country_code = icu::GetCountryCode(locale);

  bool is_ofac_sanctioned = IsISOCountryCodeOFACSanctioned(country_code);
  if (!is_ofac_sanctioned && param.should_sanction_un_m49_area_codes) {
    is_ofac_sanctioned = IsUNM49AreaCodeOFACSanctioned(country_code);
  }
...
std::string TestParamToString(
  ...
  base::ReplaceChars(locale, "-_.", "_", &locale);
  if (locale.empty()) {
    locale = "Empty";
  }

  const std::string should_sanction_un_m49_area_codes =
      test_param.param.should_sanction_un_m49_area_codes
          ? "WhenShouldSanctionUNM49AreaCodesIsSetToTrue"
          : "WhenShouldSanctionUNM49AreaCodesIsSetToFalse";

  return base::StringPrintf("%s_%s_%s", is_ofac_sanctioned.c_str(),
                            locale.c_str(),
                            should_sanction_un_m49_area_codes.c_str());
}
