...
TEST_F(BraveNewsUrlsTest, BraveNewsUsesV1ByDefault) {
  {
    brave_l10n::icu::ScopedLocaleForTesting scoped_locale_for_testing_("en_US");
  ...
  {
    brave_l10n::icu::ScopedLocaleForTesting scoped_locale_for_testing_("ja_JP");
    std::string region = brave_today::GetRegionUrlPart();
    EXPECT_EQ(brave_today::GetV1RegionUrlPart(), region);
    EXPECT_EQ("ja", region);
  }
  ...
  {
    brave_l10n::icu::ScopedLocaleForTesting scoped_locale_for_testing_("na_NA");
    std::string region = brave_today::GetRegionUrlPart();
    EXPECT_EQ(brave_today::GetV1RegionUrlPart(), region);
    EXPECT_EQ("", region);
  }
...
TEST_F(BraveNewsUrlsTest, BraveNewsV2FlagDoesNotAffectV1Region) {
  ...
  {
    brave_l10n::icu::ScopedLocaleForTesting scoped_locale_for_testing_("ja-JP");
    EXPECT_EQ("ja", brave_today::GetV1RegionUrlPart());
  }

  {
    brave_l10n::icu::ScopedLocaleForTesting scoped_locale_for_testing_("na-NA");
    EXPECT_EQ("", brave_today::GetV1RegionUrlPart());
  }
}
